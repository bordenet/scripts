...
| `origin` | `https://gitlab.int.internal.example.net/mbordenet/superpowers-plus.git` | **Internal fork** — ALWAYS synced from `upstream` after merge |
...
> ⚠️ **This workflow is SPECIFIC to `superpowers-plus` only.** Other superpowers repos (`superpowers-[removed]`, `superpowers-[removed]`) are private [COMPANY] repos — commit directly to them as normal.
